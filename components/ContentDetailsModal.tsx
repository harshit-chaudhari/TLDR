...
const PlatformLogos: Record<string, () => JSX.Element> = {
  'Netflix': () => (
    <svg viewBox="0 0 111 30" className="h-5">
  ...
  'Prime Video': () => (
    <svg viewBox="0 0 120 30" className="h-5">
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-5xl mx-4 max-h-[90vh] overflow-y-auto bg-tldr-darkGray rounded-3xl shadow-2xl">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 w-10 h-10 flex items-center justify-center rounded-full bg-black/50 hover:bg-black/70 text-white transition-all duration-200"
          aria-label="Close"
        >
        ...
        <div className="relative h-[400px] w-full overflow-hidden rounded-t-3xl">
          ...
          />
          {/* Gradient overlay */}
          <div className="absolute inset-0 bg-gradient-to-t from-tldr-darkGray via-tldr-darkGray/50 to-transparent" />
        ...
        <div className="px-8 pb-8 -mt-20 relative z-10">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap items-center gap-4 text-gray-300">
              {content.year && <span className="text-lg">{content.year}</span>}
              <span className="text-tldr-gold font-bold text-xl">
                TLDR {content.tldrRating}/10
              </span>
              <span className="px-3 py-1 bg-tldr-darkGray/50 rounded-full text-sm">
          ...
          <div className="flex flex-wrap gap-4 mb-8">
            <button
              ...
              className="group flex items-center gap-3 bg-tldr-gold text-black font-bold text-lg px-8 py-4 rounded-full hover:bg-white transition-all duration-200 transform hover:scale-105"
            >
            ...
            <a
              ...
              className="flex items-center gap-3 bg-tldr-darkGray/50 border-2 border-tldr-lightGray/30 text-white font-semibold text-lg px-8 py-4 rounded-full hover:border-tldr-gold hover:bg-tldr-darkGray transition-all duration-200"
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {content.genre && content.genre.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm uppercase tracking-wide text-gray-500 mb-3">Genres</h3>
              <div className="flex flex-wrap gap-2">
                {content.genre.map((g) => (
                  <span
                    key={g}
                    className="px-4 py-2 bg-tldr-darkGray/50 border border-tldr-lightGray/20 text-gray-300 rounded-full text-sm"
                  >
                    {g}
                  </span>
                ))}
          ...
          {/* Synopsis Section - Placeholder for TMDB data */}
          <div className="mb-6">
            <h3 className="text-sm uppercase tracking-wide text-gray-500 mb-3">Synopsis</h3>
            <p className="text-gray-300 text-base leading-relaxed">
              Detailed synopsis and plot information will be displayed here once integrated with TMDB API.
              This will include a comprehensive overview of the story, themes, and key plot points.
            </p>
          </div>

          {/* Cast & Crew - Placeholder for TMDB data */}
          <div className="mb-6">
            <h3 className="text-sm uppercase tracking-wide text-gray-500 mb-3">Cast & Crew</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {/* Placeholder for cast members */}
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="text-center">
                  <div className="w-full aspect-square rounded-full bg-tldr-darkGray/50 mb-2" />
                  <p className="text-sm text-gray-400">Cast Member {i}</p>
                  <p className="text-xs text-gray-600">Character</p>
                </div>
              ))}
            </div>
            <p className="text-xs text-gray-600 mt-4">
              Cast and crew information will be populated from TMDB API
            </p>
          </div>

          {/* Additional Info Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6 border-t border-tldr-lightGray/20">
            <div>
              <h3 className="text-sm uppercase tracking-wide text-gray-500 mb-2">Content Type</h3>
              <p className="text-white text-lg">{content.type === 'movie' ? 'Feature Film' : 'TV Series'}</p>
            </div>
            <div>
              <h3 className="text-sm uppercase tracking-wide text-gray-500 mb-2">Platform</h3>
              <p className="text-white text-lg">{content.platform}</p>
            </div>
            {content.year && (
              <div>
                <h3 className="text-sm uppercase tracking-wide text-gray-500 mb-2">Release Year</h3>
                <p className="text-white text-lg">{content.year}</p>
              </div>
            )}
            <div>
              <h3 className="text-sm uppercase tracking-wide text-gray-500 mb-2">TLDR Rating</h3>
              <p className="text-tldr-gold text-lg font-bold">{content.tldrRating}/10</p>
            </div>
          </div>
        </div>
      </div>
